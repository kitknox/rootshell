...
nonisolated enum HerdrAttachCommand {
    ...
    static func make(sessionName: String?, terminalId: String, localAttachment: LocalMultiplexerAttachment? = nil) -> String {
        let session = sessionName.map { " --session " + LoginShellCommand.singleQuoted($0) } ?? ""
        var attachment = localAttachment
        // The private UI config must win over a saved server config, while
        // HERDR_SOCKET_PATH keeps both API and client sockets pinned.
        attachment?.environment.removeValue(forKey: "HERDR_CONFIG_PATH")
        let command = attachment?.command(arguments: []) ?? "herdr\(session)"
        let script = LoginShellCommand.pathPrefix
            + "_rhc=$(mktemp /tmp/rs-herdr.XXXXXX) || exit; "
            + "trap 'rm -f \"$_rhc\"' 0; trap 'exit 129' 1; "
            + "printf '[ui]\\nmouse_capture=false\\n' >\"$_rhc\" || exit; "
            + "HERDR_CONFIG_PATH=$_rhc \(command) terminal attach "
